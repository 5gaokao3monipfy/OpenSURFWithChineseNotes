...
if (bUpright)
    ...
    co = cos(ip.orientation);%默认是旋转的
    si = sin(ip.orientation);%得到的是旋转主方向的cos ,sin值
end
...
rrx = gauss_s1 .* (-rx * si + ry * co);  rrx=reshape(rrx,cx,cy);%计算出每一个点的Harr响应值dx、dy并高斯加权处理
